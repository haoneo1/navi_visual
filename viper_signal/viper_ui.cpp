...
uint32_t viper_ui::whoami(viper_usb *pvpr)
{
  ...
  cout << "send:" << phdr->size << "," << phdr->preamble << "\n";
  cout << "size:" << cmd_size << "\n";
...
void viper_ui::read_usb_data(viper_usb *pvpr)
{ // thread to constantly poll usb for incoming data
...
void viper_ui::stream_cont(viper_usb *pvpr)
{
  ...
  while (is_continuous)
  {
    ...
    if (br && (br == (*(uint32_t *)(resp_pkg + 4) + 8)))
    { // did we read it all?
      // check crc
      crc = CalcCrc16(resp_pkg, br - 4);
      if (crc == *(uint32_t *)(resp_pkg + br - 4))
